  ...
  if (forgotLink) {
    ...
    if (modalResendBtn) {
      modalResendBtn.addEventListener('click', async function () {
        ...
        if (!result.success) {
          modalText.innerHTML = `<span style="color: #c0392b; font-weight: 600;">Error: ${result.error}</span><br><br><span style="font-size: 13px;">Please wait a minute before trying again.</span>`;
        } else {
          modalText.innerHTML = `A new password reset link has been sent to:<br><b>${result.email}</b><br><br><span style="color: #1a73e8; font-weight: 600;">Email resent successfully!</span>`;
        }
      });
    }
  }
